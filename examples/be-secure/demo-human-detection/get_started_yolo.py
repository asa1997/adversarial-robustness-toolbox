...
def plot_image_with_boxes(img, boxes, pred_cls, title, filename):
    text_size = 2
    text_th = 2
    rect_th = 2
...
eps = 32
target_y = [{"boxes": np.empty((0, 4), dtype=np.float32), "labels": np.empty(0, dtype=np.int64)}]
attack = ProjectedGradientDescent(estimator=detector, eps=eps, eps_step=2, max_iter=10)
image_adv_chw = attack.generate(x=image_chw, y=target_y)
...
plot_image_with_boxes(
    ...
    title="Predictions on original image",
...
plot_image_with_boxes(
    ...
    filename="adversarial_predictions.png",
)
